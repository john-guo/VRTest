...
StereoView::StereoView(ProxyHelper::ProxyConfig& config)	
{
	...
	switch(gameType)
	{
	case VireioD3DProxyDevice::FIXED:
		howToSaveRenderStates = HowToSaveRenderStates::STATE_BLOCK;
		break;
	case VireioD3DProxyDevice::SOURCE:
	case VireioD3DProxyDevice::SOURCE_L4D:
		howToSaveRenderStates = HowToSaveRenderStates::SELECTED_STATES_MANUALLY;
		break;
	case VireioD3DProxyDevice::UNREAL:
	case VireioD3DProxyDevice::UNREAL_MIRROR:
	case VireioD3DProxyDevice::UNREAL_UT3:
	case VireioD3DProxyDevice::UNREAL_BIOSHOCK:
		howToSaveRenderStates = HowToSaveRenderStates::STATE_BLOCK;
		break;
	case VireioD3DProxyDevice::UNREAL_BORDERLANDS:
		howToSaveRenderStates = HowToSaveRenderStates::DO_NOT_SAVE_AND_RESTORE;
		break;
	case VireioD3DProxyDevice::EGO:
	case VireioD3DProxyDevice::EGO_DIRT:
		howToSaveRenderStates = HowToSaveRenderStates::STATE_BLOCK;
		break;
	case VireioD3DProxyDevice::REALV:
	case VireioD3DProxyDevice::REALV_ARMA:
		howToSaveRenderStates = HowToSaveRenderStates::STATE_BLOCK;
		break;
	case VireioD3DProxyDevice::UNITY:
	case VireioD3DProxyDevice::UNITY_SLENDER:
		howToSaveRenderStates = HowToSaveRenderStates::STATE_BLOCK;
		break;
	case VireioD3DProxyDevice::GAMEBRYO:
	case VireioD3DProxyDevice::GAMEBRYO_SKYRIM:
		howToSaveRenderStates = HowToSaveRenderStates::STATE_BLOCK;
		break;
	case VireioD3DProxyDevice::LFS:
		howToSaveRenderStates = HowToSaveRenderStates::STATE_BLOCK;
		break;
	case VireioD3DProxyDevice::CDC:
		howToSaveRenderStates = HowToSaveRenderStates::STATE_BLOCK;
		break;
	default:
		howToSaveRenderStates = HowToSaveRenderStates::STATE_BLOCK;
...
***/
void StereoView::Draw(VireioD3DProxyResource* stereoCapableSurface)
{
	// Copy left and right surfaces to textures to use as shader input
	// TODO match aspect ratio of source in target ? 
	ID3D11Resource* leftImage = stereoCapableSurface->getActualLeft();
	ID3D11Resource* rightImage = stereoCapableSurface->getActualRight();
...
void StereoView::InitTextureBuffers()
{
	...
	m_pContext->RSGetViewports(&pNumViewports, &viewport);

	D3D11_TEXTURE2D_DESC pDesc = D3D11_TEXTURE2D_DESC();
	...
	//TODO need?
	m_pActualDevice->CreateTexture2D(&pDesc, NULL, &leftTexture);
	leftSurface = leftTexture;

	m_pActualDevice->CreateTexture2D(&pDesc, NULL, &rightTexture);
	rightSurface = rightTexture;
}
...
void StereoView::InitVertexBuffers()
{
	...
	bufferDesc.ByteWidth = sizeof(TEXVERTEX) * 4;
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = 0;
	bufferDesc.MiscFlags = 0;


	m_pActualDevice->CreateBuffer(&bufferDesc, NULL, &screenVertexBuffer);

	TEXVERTEX* vertices;
	D3D11_MAPPED_SUBRESOURCE subresource;

	m_pContext->Map(screenVertexBuffer, 0, D3D11_MAP_READ_WRITE, 0, &subresource);
	
	//screenVertexBuffer->Lock(0, 0, (void**)&vertices, NULL);
	vertices = (TEXVERTEX*)subresource.pData;
	...
	m_pContext->Unmap(screenVertexBuffer, 0);
}
